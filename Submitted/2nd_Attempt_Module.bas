...
'That way we can assign each unique value it's own row in the new data table
                    ...
                    Ticker_Unique = Cells(RowWindow, 1).Value
                    Stock_Value = Cells(DistinctV_Count, 16).Value
                    DistinctV_Count = DistinctV_Count + 1

ElseIf Cells(RowWindow, 1).Value = Ticker_Unique And Cells(DistinctV_Count - 1, 16).Value <> Stock_Value Then
Stock_Value = (Cells(DistinctV_Count - 1, 16).Value + Cells(RowWindow, 7).Value)
...
Cells(DistinctV_Count - 1, 16).Value = (Stock_Value + Cells(RowWindow, 7).Value)
